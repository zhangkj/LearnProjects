using UnityEditor;
using UnityEngine;
[CanEditMultipleObjects, CustomEditor(typeof(Star_New))]
public class StarNewInspector : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        {
            foreach (Star_New s in targets)
            {
                if (PrefabUtility.GetPrefabType(s) != PrefabType.Prefab)
                {
                    //((Star_New)target).UpdateStarNew();
                    s.UpdateStarNew();
                }
            }
        }
    ...
    void OnSceneGUI()
    {
        Star_New star = (Star_New)target;
        Transform starTransform = star.transform;
        Undo.RecordObject(star, "Move Star Point");
